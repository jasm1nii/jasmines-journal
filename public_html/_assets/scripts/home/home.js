...
{ // yipee!!!!
  ...
  function calcOffset() {
    ...
    if (window.innerWidth > 600) {
      ...
      minLeftOffset = window.innerWidth / 3;
      maxLeftOffset = window.innerWidth / 2;
      popup.style.left = `clamp(${minLeftOffset}px, 40vw, ${maxLeftOffset}px)`;

    } else if (window.innerWidth > 400) {

      minLeftOffset = window.innerWidth / 5;
      maxLeftOffset = window.innerWidth / 4;
      popup.style.left = `clamp(${minLeftOffset}px, 20vw, ${maxLeftOffset}px)`;

    } else if (window.innerWidth > 350) {

      minLeftOffset = window.innerWidth / 7;
      maxLeftOffset = window.innerWidth / 5;
      popup.style.left = `clamp(${minLeftOffset}px, 10vw, ${maxLeftOffset}px)`;
...
{ // clock
  function dateToText(date) {
    ...
    // int must be quoted to return time with leading zero:
